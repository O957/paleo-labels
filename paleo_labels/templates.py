...
def templates_ui():
    ...
    if 'template_manager' not in st.session_state:
        st.session_state.template_manager = TemplateManager(st.session_state.storage)
    
    template_manager: TemplateManager = st.session_state.template_manager
    
    # Template tabs
    tab1, tab2, tab3 = st.tabs(["Save Template", "Load Template", "Manage Templates"])
...
def save_template_ui(template_manager: TemplateManager):
    ...
    if not current_data:
        st.info("No current label data to save. Create a label first in Single Label mode.")
        return
...
def load_template_ui(template_manager: TemplateManager):
    ...
    selected_template = st.selectbox(
        "Choose Template:",
        [""] + templates,
        help="Select a template to load"
    )
    ...
    if selected_template:
        ...
        if template_record:
            ...
            st.write(f"**Created**: {template_record.get('created', 'Unknown')}")
    ...
    for template_name in templates:
        ...
        if template_record:
            ...
            with st.expander(f"📋 {template_name}"):
                ...
                with col2:
                    if st.button(f"🗑️ Delete", key=f"delete_template_{template_name}"):
...
def load_template_action(template_data: Dict):
    ...
    for i, (key, value) in enumerate(label_data.items()):
        ...
        st.session_state[f"value_{i}"] = str(value) if value is not None else ""
    ...
    st.success(f"Template loaded! Switch to Single Label mode to see it.")
    st.info("The template has been loaded into your manual entry. Go to Single Label mode to edit and use it.")


def create_template_from_current() -> Optional[Dict]:
    """
