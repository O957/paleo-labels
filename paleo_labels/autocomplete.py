...
class AutoCompleteEngine:
                ...
                for key, value in label_data.items():
                    if isinstance(key, str) and isinstance(value, str):
                        ...
                        if normalized_key and normalized_value:
                            ...
                            value_suggestions[normalized_key][normalized_value] += 1
    ...
    def get_key_suggestions(self, partial_key: str = "", limit: int = 10) -> List[str]:
        ...
        matching_keys = [
            key for key in self._key_suggestions_cache.keys()
            if partial_lower in key.lower()
        ]
        
        # Sort by frequency (most used first)
        matching_keys.sort(
            key=lambda k: self._key_suggestions_cache[k], 
            reverse=True
        )
    ...
    def get_value_suggestions(self, key: str, partial_value: str = "", limit: int = 10) -> List[str]:
        ...
        matching_values = [
            value for value in value_counter.keys()
            if partial_lower in value.lower()
        ]
        
        # Sort by frequency (most used first)
        matching_values.sort(
            key=lambda v: value_counter[v],
            reverse=True
        )
        
        return matching_values[:limit]
    
    def get_combined_suggestions(self, label_type: str, limit: int = 20) -> Dict[str, List[str]]:
    ...
    def get_smart_field_suggestions(self, current_data: Dict[str, str], label_type: str) -> Dict[str, str]:
        ...
        current_keys = set(k.lower() for k in current_data.keys())
            ...
            if current_key in field_patterns:
                for suggested_key in field_patterns[current_key]:
                    if suggested_key not in current_keys:
                        ...
                        common_values = self.get_value_suggestions(suggested_key, limit=1)
                        if common_values:
...
def render_autocomplete_text_input(
    ...
    **kwargs
    ...
    if autocomplete_engine and user_input:
        ...
        if field_type == "key":
            suggestions = autocomplete_engine.get_key_suggestions(user_input, limit=5)
        elif field_type == "value" and related_key:
            suggestions = autocomplete_engine.get_value_suggestions(related_key, user_input, limit=5)
        else:
            suggestions = []
        ...
        if suggestions:
            with st.expander("💡 Suggestions", expanded=False):
                for suggestion in suggestions:
                    if st.button(f"📝 {suggestion}", key=f"suggest_{key}_{suggestion}"):
...
def render_smart_suggestions_panel(
    ...
    """
    smart_suggestions = autocomplete_engine.get_smart_field_suggestions(current_data, label_type)
        ...
        for suggested_key, suggested_value in smart_suggestions.items():
            ...
            with col3:
                if st.button("➕", key=f"add_suggestion_{suggested_key}"):
                    ...
                    add_suggested_field_to_manual_entry(suggested_key, suggested_value)
...
def initialize_autocomplete_engine() -> AutoCompleteEngine:
    ...
    if 'autocomplete_engine' not in st.session_state:
        if 'storage' in st.session_state:
            st.session_state.autocomplete_engine = AutoCompleteEngine(st.session_state.storage)
        else:
            return None
