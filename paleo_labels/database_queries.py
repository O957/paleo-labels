...
def advanced_database_queries_ui():
    ...
    if not paleodb.is_available():
        st.warning(
            "Paleobiology database not available. Please ensure the data file exists."
        )
        return

    # Query builder tabs
    tab1, tab2, tab3 = st.tabs(
        ["Query Builder", "Saved Queries", "Export Results"]
    )
...
def query_builder_ui(paleodb: PaleobiologyDatabase):
    ...
    with col1:
        ...
        class_filter = st.selectbox(
            "Class:", [""] + paleodb.get_class_list()[:100], key="class_filter"
        )

    with col2:
        order_filter = st.selectbox(
            "Order:", [""] + paleodb.get_order_list()[:100], key="order_filter"
        )
        ...
        genus_filter = st.selectbox(
            "Genus:", [""] + paleodb.get_genus_list()[:100], key="genus_filter"
        )
...
def execute_advanced_query(
    paleodb: PaleobiologyDatabase, query: dict, limit: int
):
    """
        ...
        try:
            ...
            for rank, value in query["taxonomic_filters"].items():
                if value:
                    rank_columns = [
                        ...
                        if rank.lower() in col.lower()
                    ]
                    if rank_columns:
                        filtered_data = filtered_data.filter(
                            pl.col(rank_columns[0]).str.contains(
                                f"(?i){value}"
                            )
                        )
                ...
                if value:
                    if filter_type == "formation":
                        formation_columns = [
                            col
                            for col in data.columns
                            if "formation" in col.lower()
                            or "fm" in col.lower()
                        ]
                        if formation_columns:
                            filtered_data = filtered_data.filter(
                                pl.col(formation_columns[0]).str.contains(
                                    f"(?i){value}"
                                )
                            )
                        ...
                        locality_columns = [
                            ...
                            if any(
                                term in col.lower()
                                for term in ["locality", "location", "site"]
                            )
                        ]
                        if locality_columns:
                            filtered_data = filtered_data.filter(
                                pl.col(locality_columns[0]).str.contains(
                                    f"(?i){value}"
                                )
                            )
            ...
            if query["text_search"]:
                species_columns = [
                    ...
                    or "accepted_name" in col.lower()
                ]
                if species_columns:
                    filtered_data = filtered_data.filter(
                        pl.col(species_columns[0]).str.contains(
                            f"(?i){query['text_search']}"
                        )
                    )
...
>>>>>>> cfb802680b027920c12884c4542147bd1367ffc4
            ...
            if label_data:
                ...
                name = label_data.get(
                    "scientific_name", f"Query_Result_{imported_count + 1}"
                )
        ...
        st.success(
            f"Imported {imported_count} results to Multi-Label Session!"
        )
        st.info(
            "Switch to Batch Processing > Multi-Label Session to edit them."
        )
...
def export_results_ui():
    ...
    export_format = st.selectbox(
        "Export Format:", ["CSV", "JSON", "Labels (Multi-Session)"]
    )
    ...
    if st.button("📤 Export"):
        if export_format == "CSV":
            ...
            st.download_button(
                "📄 Download CSV", csv_data, "query_results.csv", "text/csv"
            )
