...
def process_csv_for_batch_labels(
    uploaded_file, label_type: str = "General"
...
def create_field_mapping_ui(
    csv_columns: list[str], label_type: str
) -> dict[str, str]:
    """
    ...
    for csv_col in csv_columns:
        ...
        if mapped_field:
            ...
            if label_type != "General":
                ...
                field_mapping[csv_col] = (
                    normalized if normalized else mapped_field
                )
            else:
...
def apply_field_mapping(
    label_data_list: list[dict], field_mapping: dict[str, str]
) -> list[dict]:
    """
...
def generate_batch_labels_ui():
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Multi-Label Session", "Load All Labels", "CSV Import"]
    )
...
def multi_label_session_ui():
    ...
    if st.session_state.multi_labels:
        ...
        for i, label in enumerate(st.session_state.multi_labels):
            with st.expander(
                f"📄 {label['name']}",
                expanded=i == len(st.session_state.multi_labels) - 1,
            ):
...
def render_multi_label_editor(index: int, label: dict):
    ...
    with col1:
        new_name = st.text_input(
            "Label Name:", value=label["name"], key=f"multi_label_name_{index}"
        )
    ...
    with col3:
        if st.button(
            "🗑️", key=f"delete_multi_label_{index}", help="Delete this label"
        ):
    ...
    # Add field button
    if st.button("➕ Add Field", key=f"add_field_{index}"):
    ...
    for field_key, field_value in label["data"].items():
        ...
        with col3:
            if st.button(
                "❌", key=f"del_field_{index}_{field_key}", help="Delete field"
            ):
...
def load_all_labels_ui():
    ...
    with st.expander("Preview Saved Labels", expanded=False):
        for label_name in saved_labels[:10]:  # Show first 10
            ...
            created = (
                metadata.get("created", "Unknown") if metadata else "Unknown"
            )
...
def csv_import_ui():
    ...
    if uploaded_csv:
        # Process CSV
        label_data_list, csv_columns = process_csv_for_batch_labels(
            uploaded_csv
        )
        ...
        if field_mapping:
            ...
            mapped_data_list = apply_field_mapping(
                label_data_list, field_mapping
            )
            ...
            if mapped_data_list:
                ...
                if st.button(
                    "📥 Import to Multi-Label Session", type="primary"
                ):
                    import_csv_to_multi_session(
                        mapped_data_list, batch_label_type
                    )
...
def save_batch_template(field_mapping: dict[str, str], label_type: str):
    ...
    if template_name and st.button("Save Template", key="save_template_btn"):
        ...
        if "storage" in st.session_state:
            storage: LabelStorage = st.session_state.storage
            template_file = (
                storage.user_data_dir / "templates" / f"{template_name}.json"
            )
        ...
        try:
            ...
            if label_data:
                loaded_label = {
                    ...
                    "type": metadata.get("label_type", "General")
                    if metadata
                    else "General",
                    "data": label_data,
                    "name": label_name,
...
def import_csv_to_multi_session(label_data_list: list[dict], label_type: str):
    ...
    st.success(
        f"Imported {len(imported_labels)} labels to multi-label session!"
    )
