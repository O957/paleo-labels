...
class CollectionManager:
    ...
    def load_collection(self, collection_id: str) -> dict | None:
        ...
        try:
            for collection_file in self.collections_dir.glob("*.json"):
                ...
                if (
                    ...
                    or collection.get("type") == collection_type
                ):
    ...
    def add_label_to_collection(
        self, collection_id: str, label_name: str
    ) -> bool:
    ...
    def remove_label_from_collection(
        self, collection_id: str, label_name: str
    ) -> bool:
    ...
    def search_collections(self, query: str) -> list[dict]:
        ...
        for collection in self.list_collections():
            ...
            desc_match = (
                query_lower in collection.get("description", "").lower()
            )
...
def browse_collections_ui(collection_manager: CollectionManager):
    ...
    if selected_type != "All":
        collections = [
            c for c in collections if c.get("type") == selected_type
        ]

    # Display collections
    for collection in collections:
        with st.expander(
            f"📁 {collection['name']} ({collection.get('type', 'project').title()})"
        ):
            ...
            with col1:
                ...
                st.write(
                    f"**Created**: {collection.get('created', 'Unknown')[:10]}"
                )
                st.write(f"**Labels**: {len(collection.get('labels', []))}")
                st.write(
                    f"**Subcollections**: {len(collection.get('subcollections', []))}"
                )
            ...
            with col2:
                if st.button(
                    "🗑️ Delete", key=f"delete_collection_{collection['id']}"
                ):
...
def create_collection_ui(collection_manager: CollectionManager):
    ...
    with col1:
        collection_name = st.text_input(
            "Collection Name:",
            placeholder="e.g., Morrison Formation Survey 2024",
        )
        collection_type = st.selectbox(
            "Collection Type:",
            ["project", "expedition", "site", "formation", "custom"],
        )
    ...
    parent_options = ["None"] + [
        f"{c['name']} ({c['id']})" for c in existing_collections
    ]

    parent_selection = st.selectbox(
        "Parent Collection (optional):", parent_options
    )
    parent_id = None
    ...
    if st.button("📁 Create Collection", type="primary"):
        if collection_name:
            ...
            st.success(
                f"Collection '{collection_name}' created with ID: {collection_id}"
            )
            st.rerun()
        else:
...
def manage_collection_labels_ui(collection_manager: CollectionManager):
    ...
    selected_collection_str = st.selectbox(
        "Select Collection:", collection_options
    )
    ...
    if current_labels:
        ...
        for label_name in current_labels:
            ...
            with col2:
                if st.button(
                    "❌",
                    key=f"remove_label_{label_name}",
                    help="Remove from collection",
                ):
                    if collection_manager.remove_label_from_collection(
                        collection_id, label_name
                    ):
    ...
    available_labels = [
        label for label in all_labels if label not in current_labels
    ]
    ...
    if available_labels:
        ...
        if selected_labels and st.button("➕ Add Selected Labels"):
            ...
            for label_name in selected_labels:
                if collection_manager.add_label_to_collection(
                    collection_id, label_name
                ):
                    added_count += 1
...
def collection_stats_ui(collection_manager: CollectionManager):
    ...
    with col3:
        collection_types = list(
            set(c.get("type", "project") for c in collections)
        )
    ...
    collections_by_size = sorted(
        collections, key=lambda x: len(x.get("labels", [])), reverse=True
    )

    for collection in collections_by_size[:5]:  # Top 5
        label_count = len(collection.get("labels", []))
        st.write(
            f"• **{collection['name']}**: {label_count} labels ({collection.get('type', 'project').title()})"
        )
...
def add_label_to_collection_quick(
    label_name: str, collection_id: str = None
) -> bool:
    ...
    if collection_id:
        return collection_manager.add_label_to_collection(
            collection_id, label_name
        )
